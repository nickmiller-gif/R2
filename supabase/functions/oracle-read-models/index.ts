...
import type { OracleBriefingFilter, OracleFeedHistoryFilter, OracleThemeMapFilter } from '../../../src/types/oracle/read-models.ts';
...
function createReadModelDb(client: ReturnType<typeof getSupabaseClient>): OracleReadModelDb {
  return {
    ...
    async queryThemeMap(filter?: OracleThemeMapFilter): Promise<DbOracleThemeMapRow[]> {
      let q = client.from('oracle_theme_map_read_model').select('*').order('latest_published_at', { ascending: false });
