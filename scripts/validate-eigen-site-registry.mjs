...
for (const [siteId, siteConfig] of siteEntries) {
  ...
  if (
    !siteConfig.files_dir.startsWith('knowledge/') ||
    siteConfig.files_dir.includes('..') ||
    siteConfig.files_dir.endsWith('/')
  ) {
    fail(
      `${siteId}.files_dir must stay inside knowledge/ without parent traversal or a trailing slash.`,
    );
  }
