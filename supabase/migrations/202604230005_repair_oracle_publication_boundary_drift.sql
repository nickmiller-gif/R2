-- Repair drift: migration 202604090004 is recorded as applied but some environments
-- missed DDL (oracle_signals publication columns, oracle_publication_events, read-model views).
-- This file re-applies that surface idempotently so CI typegen matches the repo contract.

-- ── oracle_signals publication columns ───────────────────────────────────────

ALTER TABLE public.oracle_signals
  ADD COLUMN IF NOT EXISTS publication_state oracle_publication_state NOT NULL DEFAULT 'pending_review',
  ADD COLUMN IF NOT EXISTS published_at timestamptz,
  ADD COLUMN IF NOT EXISTS published_by uuid REFERENCES auth.users(id) ON DELETE SET NULL,
  ADD COLUMN IF NOT EXISTS publication_notes text;

CREATE INDEX IF NOT EXISTS idx_oracle_signals_publication_state
  ON public.oracle_signals (publication_state);

CREATE INDEX IF NOT EXISTS idx_oracle_signals_published_at
...
CREATE TABLE IF NOT EXISTS public.oracle_publication_events (
...
CREATE INDEX IF NOT EXISTS idx_oracle_publication_events_target
...
DROP POLICY IF EXISTS select_oracle_publication_events ON public.oracle_publication_events;
...
DROP POLICY IF EXISTS insert_oracle_publication_events ON public.oracle_publication_events;
CREATE POLICY insert_oracle_publication_events ON public.oracle_publication_events
  FOR INSERT TO service_role
  WITH CHECK (true);

-- ── oracle_signals read policy (publication boundary + initplan-safe auth) ─
...
-- Read models (replace if already present from partial apply)
