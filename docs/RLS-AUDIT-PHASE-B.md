...
Update (2026-04-15): The `{public}` write-role hygiene finding in this audit was remediated by [supabase/migrations/202604150001_tighten_rls_public_role_write_policies.sql](../supabase/migrations/202604150001_tighten_rls_public_role_write_policies.sql). Write policies now target `authenticated`.
...
| ------------------------------- | ---------------------------------------------- | ------------------------- | ------------------------------ |
| `meg_entities`                  | `profile_id = auth.uid()`                      | All authenticated         | `{authenticated}`              |
| `meg_entity_aliases`            | Parent join → `meg_entities.profile_id`        | All authenticated         | `{authenticated}`              |
| `meg_entity_edges`              | Parent join → source `meg_entities.profile_id` | All authenticated         | `{authenticated}`              |
| `oracle_evidence_items`         | `profile_id = auth.uid()`                      | Owner only                | `{authenticated,service_role}` |
| `oracle_outcomes`               | `profile_id = auth.uid()`                      | All authenticated         | `{authenticated}`              |
...
| `oracle_thesis_knowledge_links` | Parent join → `oracle_theses.profile_id`       | All authenticated         | `{authenticated}`              |

**Verdict: ⚠️ Functional but has issues — see Findings below.**

**MEG read decision (reviewed):** `meg_entities`, `meg_entity_aliases`, and `meg_entity_edges` intentionally keep `FOR SELECT TO authenticated USING (true)` because MEG is the canonical cross-domain identity spine. Domain-level privacy is enforced at domain tables and edge RBAC, not by partitioning core MEG reads.
