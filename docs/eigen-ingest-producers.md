...
| -------------------- | --------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Ingest edge          | `supabase/functions/eigen-ingest/index.ts`                                                                            | `normalizeCorpusPolicyTags` in `_shared/eigen-policy.ts`: include **`eigen_public`** → public corpus; otherwise **`eigenx`** is enforced as the internal baseline plus your tags. Curator **topics / domain / audience** on `document.metadata` → `documents.tags` + embedding-only prefix ([eigen-curator-metadata.md](./eigen-curator-metadata.md)). |
...
| `raysretreat/eigen-raysretreat-adapter.ts`                | Default **`public`** → **`eigen_public`**; emits `site_id` **`raysretreat`** for the Ray's Retreat surface. |
...
The Ray's Retreat app smoke check in `ray-s-retreat/scripts/smoke-eigen-public.mjs` already targets `site_id: "raysretreat"`; keep that site registry entry aligned with the adapter metadata.
