...
as $body$
  ...
  scored as (
    select
      ...
      array_agg(distinct m.via) as matched_via
    from matched m
    group by m.atom_id
  )
  select
  ...
  where
    (d.lifecycle_state is null or d.lifecycle_state not in ('repealed', 'superseded', 'withdrawn'))
    and (
...
comment on function works.find_provisions_for_concepts(text[], text, int) is
  'Phase 5b: returns legislation_obligation_atoms (with provision + document context) that match any of the supplied concept_ids, ranked by how many concepts they touch. Optional fuzzy jurisdiction ILIKE filter. Excludes repealed/superseded documents. Used by agent_legislation to surface statute citations alongside the concept profile.';;
