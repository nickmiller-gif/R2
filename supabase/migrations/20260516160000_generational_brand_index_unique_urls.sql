-- Recreate generational_brand_index: count DISTINCT atlas_urls.url per brand (not per-crawl row observations).

DROP MATERIALIZED VIEW IF EXISTS public.generational_brand_index;
...
SELECT
  ...
  COALESCE(
    (
      SELECT COUNT(DISTINCT u.url)::bigint
...
COMMENT ON MATERIALIZED VIEW public.generational_brand_index IS
  'Draft Brand Index: charter spine + global document histogram + DISTINCT Atlas URL counts when charter_entities.metadata.atlas_brand_key matches atlas_crawls.brand_key.';
