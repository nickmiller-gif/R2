...
Deno.serve(
  withRequestMeta(async (req) => {
    ...
    const pending = await client
      ...
      .select(
        'claimed_at,claimed_by,correlation_id,created_at,event_type,id,idempotency_key,oracle_signal_id,payload,processed_at,source_document_id,source_ref,source_system,status',
      )
      .eq('status', 'pending')
