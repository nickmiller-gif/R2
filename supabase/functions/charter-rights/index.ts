...
const RIGHT_CREATE_FIELDS = [
  'entity_id', 'right_type', 'title', 'description',
  'effective_date', 'expiry_date', 'status', 'confidence',
] as const;

// Columns that may be updated via PATCH
const RIGHT_PATCH_FIELDS = [
  'title', 'description', 'effective_date', 'expiry_date',
  'status', 'confidence', 'reviewed_by',
] as const;
  ...
  try {
    ...
    if (req.method === 'GET') {
      if (id) {
        ...
        let query = client.from('charter_rights').select('*');
      ...
      const payload = {
        ...allowlistPayload(raw, RIGHT_CREATE_FIELDS),
      ...
      const patch = {
        ...allowlistPayload(raw, RIGHT_PATCH_FIELDS),
