...
After `supabase db push`, confirm `cron.job` contains **`r2-signal-process-dispatcher`** and `net._http_response` shows successful calls.
...
3. **Pass criterion:** queue drains so remaining pending/claimed rows approach zero within **under 5 minutes** at 15s cadence and batch 25 (worst case roughly ceil(50/25)\*15s ≈ 45s of wall time if each batch succeeds; allow headroom for cold starts).
