...
async function gatherEvidence(
  ...
  const chunkSourceAllowed =
    sourcesAllowed.length === 0 ||
  ...
  if (chunks) {
    for (const chunk of chunks) {
      evidenceRows.push({
        ...
        content_excerpt: typeof chunk.content === 'string' ? chunk.content.substring(0, 500) : null,
...
async function generateHypotheses(
  ...
  if (hypotheses.length > 0) {
    const { error } = await serviceClient.from('oracle_run_hypotheses').insert(hypotheses);
...
async function scoreAndVerify(
  ...
  for (const h of hypotheses) {
    ...
    verificationRows.push({
      ...
      adversarial_result: needsAdversarial
        ? { note: 'Adversarial check pending — requires LLM integration' }
        : null,
