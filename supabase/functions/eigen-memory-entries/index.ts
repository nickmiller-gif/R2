      ...
      if (req.method === 'GET') {
        if (entryId) {
          const { data, error } = await client
            .from('memory_entries')
            .select(
              'confidence_band,conflict_group,created_at,expires_at,id,key,owner_id,retention_class,scope,superseded_by,updated_at,value',
            )
            .eq('id', entryId)
            .single();
        ...
        if (action === 'upsert') {
          ...
          const limit =
            typeof body.limit === 'number' ? Math.max(1, Math.min(100, body.limit)) : 20;
