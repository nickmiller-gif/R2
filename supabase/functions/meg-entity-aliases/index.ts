      ...
      if (req.method === 'GET') {
        if (action === 'resolve') {
          ...
          const { data, error } = await client
            ...
            .select('alias_kind,alias_value,confidence,created_at,id,meg_entity_id,metadata,source')
          ...
          let query = client
            .from('meg_entity_aliases')
            .select(
              'alias_kind,alias_value,confidence,created_at,id,meg_entity_id,metadata,source',
            );
        ...
        const { error } = await client.from('meg_entity_aliases').delete().eq('id', deleteId);
        if (error) return errorResponse(error.message, 400);
