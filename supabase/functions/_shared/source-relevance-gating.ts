// Canonical copy for Supabase Edge Functions (Deno bundle).
//
// IMPORTANT: This is a verbatim copy of `src/lib/eigen/source-relevance-gating.ts`
// kept inside the functions tree so the Edge Function deploy does not depend on
// files outside `supabase/functions/`. Edge Functions bundle only what lives
// under their own subtree at deploy time; importing from `src/lib` couples
// deploys to the app build and invites silent regressions.
//
// If you edit this file, also edit `src/lib/eigen/source-relevance-gating.ts`
// (the app-side copy). The test at `tests/eigen/source-relevance-gating.test.ts`
// covers both paths — update it to exercise this copy if behavior diverges.
