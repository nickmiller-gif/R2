-- Entity-scoped retrieval: honor knowledge_chunks.meg_entity_id (canonical subject)
-- in addition to entity_ids JSONB overlap.
  ...
  passed AS (
    ...
    WHERE
      ...
      AND (
        ...
        OR (
          a.meg_entity_id IS NOT NULL
          AND a.meg_entity_id::text = ANY (filter_entity_ids)
        )
      )
  ),
  pass_count AS (
...
COMMENT ON FUNCTION public.match_knowledge_chunks IS
  'Cosine ANN over knowledge_chunks.embedding (HNSW), temporal bounds, entity/policy overlap (entity_ids OR meg_entity_id vs filter_entity_ids); includes oracle fields for ranking.';
