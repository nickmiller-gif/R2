CREATE OR REPLACE FUNCTION public.meg_resolve_or_create(p_entity_type text, p_canonical_name text, p_canonical_email text DEFAULT NULL::text, p_canonical_external_id text DEFAULT NULL::text, p_source_system text DEFAULT NULL::text, p_source_table text DEFAULT NULL::text, p_source_row_id text DEFAULT NULL::text, p_payload jsonb DEFAULT '{}'::jsonb)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
declare
  ...
  v_lock_key text;
begin
  -- Strip NUL bytes from text inputs: Postgres text columns cannot store them,
  -- and scraped upstream data (addresses, parcel records) occasionally contains
  -- them, which otherwise fails the canonical_name insert. btrim does not remove NUL.
  p_entity_type := replace(p_entity_type, chr(0), '');
  p_canonical_name := replace(p_canonical_name, chr(0), '');
  p_canonical_email := replace(p_canonical_email, chr(0), '');
  p_canonical_external_id := replace(p_canonical_external_id, chr(0), '');
  p_source_system := replace(p_source_system, chr(0), '');
  p_source_table := replace(p_source_table, chr(0), '');
  p_source_row_id := replace(p_source_row_id, chr(0), '');
