-- Optional hard filter on documents.tags (curator labels: topic:, domain:, etc.)
-- Replaces the 5-arg signature with a 6-arg version (last param defaults to NULL).

DROP FUNCTION IF EXISTS public.match_knowledge_chunks(double precision[], integer, text[], text[], timestamptz);
    ...
    WHERE
      ...
      AND (
        ...
        OR (
          ...
          AND a.document_tags && filter_document_tags
        )
      )
  ),
  pass_count AS (
...
COMMENT ON FUNCTION public.match_knowledge_chunks IS
  'Cosine ANN over knowledge_chunks.embedding (HNSW), temporal bounds, entity/policy overlap, optional documents.tags overlap; includes oracle_signal_id / oracle_relevance_score for ranking.';

REVOKE ALL ON FUNCTION public.match_knowledge_chunks(double precision[], integer, text[], text[], timestamptz, text[]) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.match_knowledge_chunks(double precision[], integer, text[], text[], timestamptz, text[]) TO service_role;;
